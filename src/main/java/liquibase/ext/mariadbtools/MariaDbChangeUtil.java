...
public class MariaDbChangeUtil {

    private static Logger log = Scope.getCurrentScope().getLog(MariaDbChangeUtil.class);
    ...
    public static SqlStatement[] generateStatements(MariaDbToolsChange change,
        ...
        if (change.getUseMariaDbTools() == null && !Configuration.getDefaultOn()) {
            log.fine("Not using mariadb tools, because property " + Configuration.DEFAULT_ON + " is false. " + changeSetId + ":" + change.getChangeName());
            return originalStatements;
        }
        if (change.getUseMariaDbTools() != null && !change.getUseMariaDbTools()) {
            log.fine("Not using mariadb tools, because useMariaDbTools flag is false for " + changeSetId + ":" + change.getChangeName());
            return originalStatements;
        }
        if (Configuration.skipChange(change.getChangeName())) {
            maybeLog("Not using mariadb tools, because skipChange for "
        ...
        // Note: MariaDB is a subclass of MySQLDatabase - so the MariaDbTools changes are
        // used for both MySQLDatabase and MariaDBDatabase.
        if (database instanceof MySQLDatabase) {
            if (MariaDbToolsSchemaChangeStatement.isAvailable()) {
                MariaDbToolsSchemaChangeStatement statement = new MariaDbToolsSchemaChangeStatement(
                        change.getTargetDatabaseName(),
                        change.getTargetTableName(),
                        change.generateAlterStatement(database),
                        Optional.ofNullable(change.getMariaDbToolsOptions()));
                ...
                if (Configuration.failIfNoPT()) {
                    throw new RuntimeException("No mariadb tools found!");
                }
                maybeLog("Not using mariadb tools, because it is not available!");
            }
        }
     ...
     */
    public static String resolveReferencedMariaDbToolsTableName(String baseTableName, String referencedTableName) {
        if (baseTableName != null && baseTableName.equals(referencedTableName)
                && (!MariaDbToolsSchemaChangeStatement.getVersion().isGreaterOrEqualThan("2.2.21")
                        || MariaDbToolsSchemaChangeStatement.getVersion().isGreaterOrEqualThan("3.0.0"))) {
