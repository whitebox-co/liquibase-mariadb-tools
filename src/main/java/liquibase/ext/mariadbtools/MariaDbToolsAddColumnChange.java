 ...
 */
@DatabaseChange(name = MariaDbToolsAddColumnChange.NAME, description = "Adds a new column to an existing table",
    priority = MariaDbToolsAddColumnChange.PRIORITY, appliesTo = "table")
public class MariaDbToolsAddColumnChange extends AddColumnChange implements MariaDbToolsChange {
    ...
    private String addForeignKeyConstraint(AddColumnConfig column, Database database) {
        ...
        if (constraintsConfig != null && (StringUtil.isNotEmpty(constraintsConfig.getReferences()) || StringUtil.isNotEmpty(constraintsConfig.getReferencedTableName()) )) {
            ...
            referencedTable = MariaDbChangeUtil.resolveReferencedMariaDbToolsTableName(getTableName(), referencedTable);
    ...
    protected Change[] createInverses() {
        ...
        for (ColumnConfig aColumn : getColumns()) {
            ...
            // that's the mariadb tools drop column change.
            MariaDbToolsDropColumnChange inverse = new MariaDbToolsDropColumnChange();
