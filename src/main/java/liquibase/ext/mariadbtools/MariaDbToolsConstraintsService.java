...
public class MariaDbToolsConstraintsService {
    private static MariaDbToolsConstraintsService instance = new MariaDbToolsConstraintsService();
    private Logger log = Scope.getCurrentScope().getLog(MariaDbToolsConstraintsService.class);
    private boolean enabled = true;

    public static MariaDbToolsConstraintsService getInstance() {
        return instance;
    }
     ...
     */
    public String determineCurrentConstraintName(Database database, MariaDbToolsDropForeignKeyConstraintChange change) {
        // start with best guess without looking at the database
        String constraintName = change.getConstraintName();

        if (enabled && !MariaDbChangeUtil.isDryRun(database) && MariaDbChangeUtil.isConnected(database)) {
            constraintName = findForeignKey(database, change);
        }
        return prefixConstraintName(constraintName);
    }

    private String findForeignKey(Database database, MariaDbToolsDropForeignKeyConstraintChange change) {
     ...
     */
    public boolean hasPrimaryKey(Database database, MariaDbToolsAddPrimaryKeyChange change) {
