...
public class ChangeLogParserTest {
    ...
    public void setup() throws IOException {
        ...
        data.put("test-changelog.xml",
                FileUtil.getContents(new File("src/test/resources/liquibase/ext/mariadbtools/changelog/test-changelog.xml")));
        data.put("test-changelog.yaml",
                FileUtil.getContents(new File("src/test/resources/liquibase/ext/mariadbtools/changelog/test-changelog.yaml")));
        data.put("raw.githubusercontent.com/liquibase/liquibase-mariadbtools/liquibase-mariadbtools-2.0.0/src/main/resources/dbchangelog-ext-liquibase-mariadbtools.xsd",
                FileUtil.getContents(new File("src/main/resources/dbchangelog-ext-liquibase-mariadbtools.xsd")));
    ...
    private static void assertChange(Change change, Class<? extends MariaDbToolsChange> type, Boolean usePercona,
                                     String perconaOptions) {
        Assertions.assertEquals(type, change.getClass());
        Assertions.assertEquals(usePercona, ((MariaDbToolsChange)change).getUseMariaDbTools());
        Assertions.assertEquals(perconaOptions, ((MariaDbToolsChange)change).getMariaDbToolsOptions());
    }
    ...
    private static void assertChangeLog(DatabaseChangeLog changelog) {
        ...
        assertChange(change, MariaDbToolsAddColumnChange.class, Boolean.FALSE, null);
        change = changelog.getChangeSets().get(2).getChanges().get(0);
        assertChange(change, MariaDbToolsAddColumnChange.class, null, "--foo");
    }
